...
exports.getPatientHistory = async (req, res) => {
  try {
    ...
    const totalPatients = await Appointment.distinct('userId', { doctorId }).then(ids => ids.length);
...
exports.getDoctorAvailability = async (req, res) => {
  try {
    ...
    const doctor = await Doctor.findById(doctorId).select('name specialization availableDays timeSlots');
    if (!doctor) {
    ...
    if (date) {
      ...
      const availableSlots = doctor.timeSlots.filter(slot => !bookedTimes.includes(slot.startTime));
